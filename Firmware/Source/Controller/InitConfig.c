...
void INITCFG_ConfigDMA()
{
	...
	DMAChannelX_DataConfig(DMA_ADC_DUT_U_CHANNEL, (uint32_t)(&LOGIC_ADC_VoltageRaw[0]), (uint32_t)(&ADC1->DR), PULSE_ARR_MAX_LENGTH);
	...
	DMAChannelX_Config(DMA_ADC_DUT_I_CHANNEL, DMA_MEM2MEM_DIS, DMA_LvlPriority_LOW, DMA_MSIZE_16BIT, DMA_PSIZE_16BIT,
							DMA_MINC_EN, DMA_PINC_DIS, DMA_CIRCMODE_EN, DMA_READ_FROM_PERIPH);
	DMAChannelX_DataConfig(DMA_ADC_DUT_I_CHANNEL, (uint32_t)(&LOGIC_ADC_CurrentRaw[0]), (uint32_t)(&ADC2->DR), PULSE_ARR_MAX_LENGTH);
